...
evictEntry e@(TimeEntry key value _) = do
  liftIO $ debugM "TimeCache" $ "Evicting: " ++ (C.unpack value) ++ " ... "
  deleteEntry key

  resp <- post value
  if isRight resp
    then liftIO $ debugM "TimeCache" " OK."
    else liftIO $ debugM "TimeCache" " Fail."

cacheEntry :: TimeEntry -> TimeCache ()
cacheEntry entry@(TimeEntry key value time) = do
  liftIO $ debugM "TimeCache" $ "Caching: " ++ show entry
...
deleteEntry key = do
  liftIO $ debugM "TimeCache" $ "Deleting key: " ++ show key
  deleteKey key
...
getEntries :: TimeCache [TimeEntry]
getEntries = do
  mkvStore <- getKVStore
  liftIO $ atomically $ do
    kvStore <- readTVar mkvStore
    return $ M.elems kvStore
